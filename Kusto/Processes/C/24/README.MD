...
**При завантаженні форми виконується сценарій**: 
	- `Z10T10_OnFormLoad(Context context, FormViewBuilder form)`

**Вихідні переходи**:
	- `ОК` - При натисканні кнопки `На согласование` — Відображується спливаюча форма `Z10T10_A10__OK` з відображенням подальших учасників процесу: Перевіряється заповнення всіх необхідних полей  
	- `Save`    
	- `Abort`    
	- Событие 4    
